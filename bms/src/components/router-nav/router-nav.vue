<template>
    <el-menu :default-active="$route.path" :default-openeds="level1" theme="dark" router>
        <template v-for="route in $router.options.routes">
            <div  v-if="!route.meta.hidden" :key="route.child">
                ...
                <el-submenu v-else :index="route.path">
                    ...
                    <el-menu-item  v-if="!child.meta.hidden" :index="route.path + '/' + child.path" :key="route.path + '/' + child.path">{{ child.meta.name }}</el-menu-item>
                    </template>
                </el-submenu>
            </div>
        </template>
